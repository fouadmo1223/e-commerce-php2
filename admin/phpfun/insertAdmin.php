<?php
    ...
    if( strlen($username) > 20 ){
        ...
        $selectUserName = $connection -> query("SELECT * FROM admins WHERE username = '$username'");
    ...
    if( strlen($email) < 5 ){
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){
            $selectEmail= $connection -> query("SELECT * FROM admins WHERE email = '$email'");
    ...
    if($passwordsucc && $userNamesucc && $fullNamesucc && $emailsucc){
        ...
        $insertAdmin = "INSERT INTO admins (fullname,username, email,password,permission_id,gender_id) VALUES ('$fullname','$username', '$email', '$password','2','$gender')";
        $insertion = $connection ->query($insertAdmin);
        if($insertion ){
            $_SESSION["formValid"] = true;
            header("Location:../Admins.php?action=addAdmin");
            exit();
        }else{
